  ...
  return (
    <div className={`fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4 backdrop-blur-sm ${animationClasses}`} onClick={handleClose}>
      <div className={`w-full max-w-lg bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-xl shadow-2xl p-6 ${contentAnimationClasses}`} onClick={(e) => e.stopPropagation()}>
        <h2 className="text-2xl font-bold text-white mb-4">Log Mock Test Result</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            ></textarea>
            <p className="text-xs text-gray-500 mt-1">These will be added to your Priority Weaknesses.</p>
          </div>
          <div className="flex justify-end gap-4 pt-4">
            ...
            <button type="submit" className="px-5 py-2 text-sm font-semibold rounded-lg bg-gradient-to-r from-cyan-500 to-purple-500 text-white hover:opacity-90">Log Result</button>
          </div>
        </form>
      </div>
    </div>
