...
# Version: 6.0.0 (JSON Only)
# Purpose: To guide AI agents in generating structured JSON data for the JEE Scheduler Pro platform and integrating with it.
...
- **Example with Answer Key & Flashcards:** "Create a homework for JEE Mains 2023 Jan 29 Shift 1 paper, find its official answer key, and generate 5 flashcards on the toughest topics from that paper."
...
| \`flashcards\`| array  | **For \`HOMEWORK\` only.** Optional. An array of objects: \`[{ "front": string, "back": string }]\`. | \`[{"front": "Formula for Torque?", "back": "τ = r × F"}]\` |
...
---
## 5. FULFILLMENT: Methods for Data Integration

### 5.1 Method A: Direct PWA Integration (for Google Assistant)
This method uses a deep link to open the PWA and automatically import data. It is the standard for voice-based actions.
...
### 5.2 Method B: Secure API Endpoint (for Automated Systems)
This method allows automated scripts or other services to push data directly to a user's account without manual copy-pasting.

**Endpoint URL:**
\`https://[YOUR_PWA_DOMAIN]/api/import\`
...
**Body:**
- The raw JSON object as specified in sections 3 and 4.

**Instructions:**
1.  The user must first generate a personal API token from within the JEE Scheduler Pro app's profile settings.
2.  You must obtain this token from the user and include it in the \`Authorization\` header.
3.  Generate the complete JSON object.
4.  Send a \`POST\` request to the endpoint with the JSON as the request body.`;
